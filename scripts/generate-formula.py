#!/usr/bin/env python3
"""Generate the git-rg Homebrew formula from a verified GitHub Release."""
...
def fetch(url: str) -> bytes:
    ...
    headers = {"Accept": "application/vnd.github+json", "User-Agent": "git-rg-homebrew-tap"}
    if token:
...
def release_assets(release: dict[str, object]) -> dict[str, str]:
    ...
    required = [template.format(tag=tag) for template in ARCHIVES.values()]
...
def verify_assets(tag: str, assets: dict[str, str]) -> dict[str, str]:
    ...
    names = [template.format(tag=tag) for template in ARCHIVES.values()]
...
def formula(tag: str, version: tuple[int, int, int], checksums: dict[str, str]) -> str:
    ...
    linux_amd64 = values("linux_amd64")
    linux_arm64 = values("linux_arm64")
    darwin_amd64 = values("darwin_amd64")
    darwin_arm64 = values("darwin_arm64")
    return f'''class GitRg < Formula
  ...
  on_macos do
    if Hardware::CPU.arm?
      url "{darwin_arm64[0]}"
      sha256 "{darwin_arm64[1]}"
    elsif Hardware::CPU.intel?
      url "{darwin_amd64[0]}"
      sha256 "{darwin_amd64[1]}"
    else
  ...
  on_linux do
    if Hardware::CPU.arm?
      url "{linux_arm64[0]}"
      sha256 "{linux_arm64[1]}"
    elsif Hardware::CPU.intel?
      url "{linux_amd64[0]}"
      sha256 "{linux_amd64[1]}"
    else
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=Path("Formula/git-rg.rb"))
    args = parser.parse_args()
    try:
        tag, version, release = select_release()
        assets = release_assets(release)
        checksums = verify_assets(tag, assets)
        content = formula(tag, version, checksums)
        args.output.parent.mkdir(parents=True, exist_ok=True)
        temporary = args.output.with_name(f".{args.output.name}.tmp")
        temporary.write_text(content, encoding="utf-8")
        temporary.replace(args.output)
    except GenerationError as exc:
        print(f"generate-formula: {exc}", file=sys.stderr)
        return 1
    print(tag)
    return 0
